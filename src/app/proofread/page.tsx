...
function ProofreadComponent() {
  ...
  const handleProofread = async () => {
    ...
    setResultText("");
    try {
      const result = await proofreadText({ text: inputText });
      setResultText(result.proofreadText);
      setStatus("success");
    } catch (error) {
    ...
    <>
      ...
      <main className="flex-grow container mx-auto p-4 md:p-8">
        <div className="text-center mb-8">
          <h2 className="text-3xl font-headline text-primary">{t('proofread.title')}</h2>
          <p className="text-muted-foreground">{t('proofread.description')}</p>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          <Card className="w-full shadow-lg">
            <CardHeader>
              <CardTitle>{t('proofread.pasteTitle')}</CardTitle>
            </CardHeader>
            <CardContent>
              <Textarea
                value={inputText}
              ...
              <div className="flex items-center my-4">
                ...
                <span className="mx-4 text-muted-foreground font-bold">{t('proofread.or')}</span>
                <Separator className="flex-grow" />
              </div>
              {fileName ? (
                ...
                >
                    ...
                    <p className="font-semibold">{t('proofread.uploadCta')}</p>
                    <p className="text-sm text-muted-foreground">{t('proofread.uploadSupports')}</p>
                    <input
                    type="file"
                    ref={fileInputRef}
          ...
          <Card className="w-full shadow-lg">
            <CardHeader>
              <CardTitle>{t('proofread.resultTitle')}</CardTitle>
            </CardHeader>
            <CardContent>
              {status === 'processing' && (
                <div className="flex flex-col items-center justify-center h-96">
                  <Loader2 className="w-16 h-16 text-primary animate-spin mb-4" />
                  <p className="mb-2">{t('proofread.processing')}</p>
                </div>
              )}
              {status !== 'processing' && (
